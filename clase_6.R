...
biocli_DF <- Bioclimatic %>% as("SpatialPixelsDataFrame") %>% 
        as.data.frame()

#extrayendo la T media

biocli_DF <- biocli_DF %>% select(bio1, x, y)

ggplot() +  geom_tile(data = biocli_DF, aes(x= x, y =y, fill = bio1)) + geom_sf(data = N_pumilio_SF) +
...
Aus <- dismo::randomPoints(mask = Bioclimatic[[1]], n = 5000) %>% #usualemnte 10000
...
predic_DF <- Prediction %>% as("SpatialPixelsDataFrame") %>% 
        as.data.frame()

ggplot() + geom_tile(data = predic_DF, aes(x = x, y = y, fill=layer))+ scale_fill_viridis_c(name="Probabilidad")
